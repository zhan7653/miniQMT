...
已在真实中转预演与本地决策校验通过后启用周度调用。新闻输入与自主学习仍是后续阶段。
...
- 关键行情字段(OHLCV)必须有**两个独立后端**佐证,冲突由第三源仲裁;不能确认的价格只允许以精确标的/日期的无价格 quarantine 发布,不得冒充可信 OHLCV。
...
- **按故障域 fail-closed**:能定位到标的、日期、证据或账户的失败只冻结该范围及真实依赖,其余范围继续;只有日历、schema、manifest/CAS 等不可切分的共享事实不可信时才暂停新交易日,并保留上一可信快照和账户 head。
...
| 故障域隔离 fail-closed | 存疑范围停止并记录原因;只冻结精确范围及依赖,共享不变量失败才暂停整次新发布 |
